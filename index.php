<?php
// Coding for CRUD on eTimeTable for Admin Timetable
...
if(isset($_POST['action']) or isset($_GET['view'])) //show all events
{
    if(isset($_GET['view']))
    {
        ...
        $result = mysqli_query($dbcon,"SELECT id, start ,end ,title FROM  etimetable where (date(start) >= '$start' AND date(start) <= '$end')");
    ...
    {   
        mysqli_query($dbcon,"INSERT INTO etimetable(
                    title ,
                    start ,
                    end 
                    )
    ...
    {
        mysqli_query($dbcon,"UPDATE etimetable set 
    ...
    {
        mysqli_query($dbcon,"DELETE from etimetable where id = '".mysqli_real_escape_string($dbcon,$_POST["id"])."'");
